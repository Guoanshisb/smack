...
std::string SmackRep::getPrelude() {
  ...
  if (ptrSizeInBits == 32)
    ...
    llvm_unreachable("Unexpected point bit width.");
  s << Decl::function(indexedName("$bv2int",{ptrSizeInBits}), {{"i",bt}}, it, Expr::cond(Expr::fn(indexedName("$slt", {bt, "bool"}), {arg, Expr::lit(0UL, ptrSizeInBits)}), Expr::fn("$sub.ref", {uint, Expr::lit(sub, 0U)}), uint), {Attr::attr("inline")});
  s << "\n";
